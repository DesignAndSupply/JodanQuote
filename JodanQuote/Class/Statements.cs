...
namespace Statements
{
    class Statementsclass
    {
        ...
        public static string Search_stock_category = "SELECT * from C_view_hardware WHERE category = @category AND jodan_stock = 1";
        public static string Search_stock_category_description = "SELECT * from C_view_hardware WHERE category = @category AND Description LIKE '%' + @description + '%'  AND jodan_stock = 1";
        public static string Search_stock_description = "SELECT * from C_view_hardware WHERE Description LIKE '%' + @description + '%' AND jodan_stock = 1  ";
        ...
        public static string revise_item = "Insert into dbo.item (item_id ,project_id , item_date, revision_id,material_id,finish_id,door_ref,door_type_id,door_style,structual_op_height,structual_op_width,frame_width,frame_height,jamb_width,jamb_height,finish_description,material_thickness,created_by," +
                                           ...
                                           "Values (@item_id ,@project_id , @item_date, @revision_id,@material_id,@finish_id,@door_ref,@door_type,@door_style,@structual_op_height,@structual_op_width,@frame_width,@frame_height,@jamb_width,@jamb_height,@finish_description,@material_thickness,@created_by," +
        ...
        public static string update_quotation_item = "Update dbo.item SET structual_op_height = @structure_height, structual_op_width = @structure_width, fire_rating_cost = @fire_rating_cost, security_rating_cost = @security_rating_cost,jamb_style_id = @jamb_style_id, jamb_width = @jamb_width, jamb_height = @jamb_height," +
                                                     "frame_height = @frame_height, frame_width = @frame_width , door_type_id = @door_type, finish_id = @finish_id, material_id = @material_id, fire_rating_id = @fire_id, security_rating_id = @security_rating_id, material_thickness = @material_thickness,item_notes = @item_notes," +
        ...
        public static string update_stock_null = "Update dbo.Stock Set jodan_stock = NULL WHERE id = @id";
        //email
